...
----
java -jar plexusgen.jar --type=ts --baseDir=./quick-start/metadata --input=WebCcyPairRateViewer.interop --out=./quick-start/web/ccy-pair-rate-provider --protoc=./quick-start/web/ccy-pair-rate-provider/node_modules/.bin/pbts.cmd
----
...
----
    .withCcyPairRateServiceInvocationsHandler({
        ...
        onGetExpiringRate: async (ccyPair: plexus.fx.ICcyPair) => {
            log.info(`Received request for ${ccyPair.ccyPairName} CCY Pair Rate`);
            return {
