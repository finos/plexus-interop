...
Plexus Interop standard spans several layers and concepts. This section contains overview diagram which shows how
layers and concepts are related to each other on example of 2 applications calling each other through broker. Below
this diagram you can find descriptions for all these layers and concepts.
...
Component inside broker which tracks active connections and can trigger launch of an application on demand by sending
request to the appropriate app launcher.
