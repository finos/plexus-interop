...
We changed provided service definition, so now we need to re-generate Provider's client and provide implementation for new provided method:
...
. Open `web/packages/ccy-pair-rate-provider/src/index.ts` and change previously provided `onGetRate` implementation to `onGetRateStreaming` as follows:
+
[source,javascript]
-----
