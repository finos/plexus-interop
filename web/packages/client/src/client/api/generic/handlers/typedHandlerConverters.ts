...
import { GenericUnaryInvocationHandler } from '../';

export function toGenericUnaryHandler(
    handler: UnaryInvocationHandler<any, any>,
    requestType: any,
    responseType: any,
    marshallerProvider: MarshallerProvider): GenericUnaryInvocationHandler {
    ...
    return {
        ...
        handler: {
            ...
            handle: async (invocationContext, request: ArrayBuffer) => {
                ...
                const rawResponse = await handler.handler.handle(invocationContext, payload);
